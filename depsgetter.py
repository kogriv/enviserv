...
class DepsGetter:
    """
    Class to find and check python virtual
    environments and dependencies.
    Checking current used venv and package manager
    (from conda, venv, virtual env).
    Specified for working in docker container scope.
    """
    def __init__(self):
    ...
    def run_autoshell_command(self, command, verbose=False):
        """
        Executes a shell command using subprocess.Popen and returns the output.
        ...
        Side Effects:
            ...
            - If an error occurs, it stores the error in self.errors and optionally prints it.
        """
        process = subprocess.Popen(
                    command,
                    stdout=subprocess.PIPE,
    ...
    def get_base_win_paths(self, verbose=False):
        """
        Retrieves the base Windows paths for Python installations and extracts their versions.

        Args:
            verbose (bool): If True, prints additional information. Default is False.

        Side Effects:
            - Updates self.grouped_paths with Python versions and their corresponding paths.
            - Optionally prints the paths and versions found.
        """
    ...
    def get_base_win_deps(self,env_name,verbose=False):
        """
        Retrieves the dependencies for a given Python virtual environment.
        ...
        Returns:
            dict: A dictionary of dependencies with package names as keys and versions as values.

        Side Effects:
            - Updates self.errors with any exceptions that occur.
            - Optionally prints the command and its output.
        """
    ...
    def check_library_in_current_venv(self, library_name,verbose=False):
        """
        Checks if a specified library is installed in the current virtual environment.
        ...
        Returns:
            int: 1 if the library is installed, 0 otherwise.

        Side Effects:
            - Optionally prints the library version or a message indicating the library is not found.
        """
        if verbose:
    ...
    def count_specifed_venv_deps_is_in_current_venv(self,
                                    ...
                                    verbose=False):
        """
        Counts the specified dependencies that are installed in the current virtual environment.

        Args:
            deps (dict): A dictionary of dependencies to check.
            verbose (bool): If True, prints additional information. Default is False.

        Returns:
            tuple: A tuple containing the number of specified dependencies found and the total number of specified dependencies.

        Side Effects:
            - Optionally prints the count of installed dependencies.
        """
        if deps is None:
            if verbose:
    ...
    def get_current_dependencies(self,verbose=False):
        """
        Retrieves the current dependencies installed in the virtual environment.

        Args:
            verbose (bool): If True, prints additional information. Default is False.

        Returns:
            dict: A dictionary of current dependencies with package names as keys and versions as values.

        Side Effects:
            - Optionally prints information about exceptions that occur during retrieval.
        """
        dependencies = {}
        try:
    ...
    def count_current_deps_is_in_specifed_venv(self, some_venvs_deps=None,current_deps=None, verbose = False):
        """
        Counts the current dependencies that are also present in the specified virtual environment's dependencies.

        Args:
            some_venvs_deps (dict): A dictionary of some virtual environment's dependencies.
            current_deps (dict): A dictionary of the current environment's dependencies.
            verbose (bool): If True, prints additional information. Default is False.

        Returns:
            tuple: A tuple containing the number of current dependencies found in the specified environment and the total number of current dependencies.

        Side Effects:
            - Optionally prints the comparison results.
        """
    ...
    def print_dashes(self, symb="-", m=3, n=5, delay=100, increase = 1.3):
        """
        Prints a series of lines with increasing length.

        Args:
            symb (str): The symbol to use for the lines. Default is "-".
            m (int): The initial number of symbols per line. Default is 3.
            n (int): The number of lines to print. Default is 5.
            delay (int): The delay between lines in milliseconds. Default is 100.
            increase (float): The factor by which to increase the line length. Default is 1.3.

        Side Effects:
            - Prints lines with increasing length.
        """
        print("*")
        for i in range(1,n):
            print((symb * m)*i)
    ...
    def check_current_venv_path(self, update_data = False, verbose=False,
        ...
        """
        Checks the current virtual environment path and updates the virtual environment paths and dependencies if needed.

        Args:
            update_data (bool): If True, updates all dependencies data. Default is False.
            verbose (bool): If True, prints additional information. Default is False.
            symb (str): Symbol to use for printed lines. Default is "-".
            m (int): Initial number of symbols per line. Default is 5.
            n (int): Number of lines to print. Default is 5.
            delay (int): Delay between lines in milliseconds. Default is 100.

        Returns:
            str or bool: The path to the virtual environment activation script if the environment matches; 
                        otherwise, False.

        Side Effects:
            - Calls self.print_dashes() to print lines.
            - Calls self.get_venvs_paths() and self.get_conda_envs() to get paths to virtual environments.
            - Calls self.get_all_dependencies() to retrieve all dependencies if update_data is True.
            - Calls self.get_current_dependencies() to retrieve current environment dependencies.
            - Calls self.count_specifed_venv_deps_is_in_current_venv() and self.count_current_deps_is_in_specifed_venv() 
            for dependency comparisons.
            - Prints detailed information if verbose is True.
        """
